...
def pred(liste):
    ...
    # récupérer les 10 premières entrées de la liste
    ls = []
    for i in range(0,10):
        ls.append(liste[i])

#    mettre le liste en dataframe pour utiliser la bibli pandas
    df = pd.DataFrame(ls)
    caracteres1 = {
        'a': 0,
        'b': 1,
    ...
    for ligne in range(df.shape[0]):
        df.iloc[ligne] = df.iloc[ligne].map(caracteres1)
        
    # mettre les valeurs en int au cas ou    
    df.iloc[:,0] = pd.to_numeric(df.iloc[:,0])
    
    # transposer en np.array et changer les axes du vecteur
    df = np.array(df)
    df = np.transpose(df)
    
    # effectuer la prédiction
    clf = load('regression_model_saved.joblib')
    res = clf.predict(df)[0]
    
    # retransformer les résultats en lettres
    if res == 0:
        res = 'A'
    elif res == 1:
        res = 'B'
...
def run_quiz(questions, Interpretation):
    ...
    #  print('-----------------------------------------')
     ...
     try:
        res = pred(Data)
        chaine = f'interprétation réelle : {Data[11]}\nprédiction : {res}'
        return print(chaine)
     except:
         return print('Vous avez des données erronées qui ne peuvent pas être traitées')
